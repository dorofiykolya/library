...
{
	import common.composite.EventDispatcher;
	...
	public class Component extends EventDispatcher
	{
		...
		internal static function dispose(item:Component):void
		{
			if (!item._disposed)
			{
				item._disposed = true;
		...
		public function broadcastEventWith(type:String, bubbles:Boolean = false, data:Object = null):void
		{
			if (_entity)
			{
				_entity.broadcastEventWith(type, bubbles, data);
			}
		}
		
		public function broadcastEventAs(typeEvent:Class, type:String, ... args):void
		{
			if (_entity)
			{
				_entity.broadcastEventAs(typeEvent, type, args);
			}
		}
